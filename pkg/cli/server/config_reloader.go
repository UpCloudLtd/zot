...
func (hr *HotReloader) reloadConfig(reason string) {
	...
	if err != nil {
		...
		// a load that failed reading the credentials file still decoded where
		// it lives, so follow it: otherwise creating that file moves no
		// fingerprint and the config is never retried
